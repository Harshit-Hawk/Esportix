...
export default function CreateTournamentPage() {
  ...
  const handleGameSelect = (gId: string) => {
    setGameId(gId);
    const selectedGame = games.find((g) => g.id === gId);
    if (selectedGame) {
      if (selectedGame.slug === "bgmi") setSelectedPresetKey("bgmi_official_10pt");
      else if (selectedGame.slug === "free-fire") setSelectedPresetKey("free_fire_official");
      else if (selectedGame.slug === "valorant") setSelectedPresetKey("valorant_tourney");
      else if (selectedGame.slug === "cod-mobile") setSelectedPresetKey("cod_mobile_br");
      else setSelectedPresetKey("custom_customizable");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase.from("tournament_audit_logs").insert({
        ...
        new_value: { name, slug, preset: preset.name },
      });
      ...
      <div className="rounded-xl border border-esports-navy-border bg-esports-navy-card p-6 sm:p-8 shadow-2xl space-y-6">
        <div className="border-b border-esports-navy-border pb-4">
          ...
          <p className="text-xs text-esports-silver mt-1">
            Configure tournament branding, game title, and scoring presets.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Free Fire All-Stars Championship 2026"
                value={name}
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="free-fire-all-stars-2026"
                value={slug}
          ...
          <div>
            <label className="block text-xs font-black uppercase tracking-wider text-esports-silver mb-2 flex items-center gap-1.5">
              ...
              <span>Initial Scoring Rule Preset *</span>
            </label>
